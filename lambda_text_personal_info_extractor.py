...
def lambda_handler(event, context):
    """
    AWS Lambda 함수: OCR로 추출된 텍스트에서 개인정보 추출
    (OCR은 기존 ocr_service.py에서 처리하고, 이 Lambda는 텍스트에서 개인정보만 추출)
    """
    try:
        # 입력 데이터 파싱
        if 'body' in event:
        ...
        if not extracted_text:
            return {
                'statusCode': 400,
                'body': json.dumps({
                    'error': 'extracted_text는 필수입니다'
                }, ensure_ascii=False)
            }
        
        logger.info(f"개인정보 추출 시작 - 문서 타입: {document_type}, 텍스트 길이: {len(extracted_text)}")
        
        # 개인정보 추출
        personal_info = extract_personal_info(extracted_text, document_type)
...
def extract_personal_info(extracted_text: str, document_type: str) -> Dict[str, Any]:
    """
    Bedrock을 사용하여 추출된 텍스트에서 개인정보 추출
    """
    try:
        model_id = 'anthropic.claude-3-5-sonnet-20240620-v1:0'
        
        prompt = create_prompt(extracted_text, document_type)
        ...
        logger.error(f"개인정보 추출 오류: {str(e)}")
        raise ValueError(f"개인정보 추출 실패: {str(e)}")

def create_prompt(extracted_text: str, document_type: str = 'general') -> str:
    """
